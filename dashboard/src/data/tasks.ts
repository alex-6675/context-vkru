...
export const tasks: TaskItem[] = [
  ...
  },
  {
    ...
    commit: "fix(ext): v07f6 — метка на каждое вхождение, без предохранителей [TASK-0011]",
    title: "v07f6: метка на каждое вхождение (без «одна на контейнер» и «≤3»), NAME_HINT только если пусто",
    status: "queue",
    statusLabel: "NEW · исполнено · ждёт хэша",
    dep: "v07f5 (FAIL: предохранители съедали метки)",
    goal: "Убрать предохранители «одна метка на контейнер» и «≤3 на страницу», которые съедали метки в плотных видах (мессенджер, треды). Метить КАЖДЫЙ текстовый якорь сохранённой карточки (один ▲ на вхождение). NAME_HINT: displayName — только если пуст, иначе «имя НЕ тронуто»; identity.name — только если пуст; запись в db только при изменении. Окна, логи, тип, живая перекраска — как в v07f5.",
    files: [
      "src/content.js — scan() без perCard/cardCount и лимита 3; метим каждое вхождение; containerOf удалена",
      "src/background.js — handleNameHint: displayName/identity.name только если пусты; лог «имя НЕ тронуто»; запись при изменении",
      "src/core/messaging.js — CTX_BUILD «v07f6»; manifest.json — 0.0.16",
    ],
    test: [
      "Плотный вид (мессенджер/тред): ▲ у КАЖДОГО вхождения сохранённой карточки, не «исчезает» после третьего",
      "После обновления страницы метки возвращаются у всех вхождений",
      "NAME_HINT: у карточки с заданным именем displayName НЕ перезаписывается («имя НЕ тронуто: cN (уже задано)»)",
      "У карточки без имени имя подставляется из первого якоря («имя сохранено»)",
    ],
    pass: "Пункты 1–4. R1/R2 в этот этап не входят (→ TASK-0012/v08r).",
    basis: "сводка проектировщика · фикс v07f6 (v07f5 = FAIL: предохранители) · Дополнение №3",
  },
];
