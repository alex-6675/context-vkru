...
export const tasks: TaskItem[] = [
  ...
  },
  {
    ...
    commit: "fix(ext): v07f4 — единственный писатель, без воскрешений [TASK-0011]",
    title: "v07f4: единственный писатель (read-modify-write), без воскрешений",
    status: "queue",
    statusLabel: "NEW · исполнено · ждёт хэша",
    dep: "v07f3 (FAIL по D18)",
    goal: "Идемпотентный рендер (снятие span.ctx-hl по классу + selfChange); контейнеры li→post→comment→dialog→section→body, одна ▲ на карточку на контейнер, ≤3 на страницу; единый серый треугольник #8a94a3 (ctx-mark-c убран); заливка букв ника цветом карточки (~80%); удостоверение без мусора (cleanUrl: reply/thread/w); секция «Точка встречи и история» + селект «ответ последнего контакта». Одно окно, удаление, NAME_HINT, живой onChanged — сохранены.",
    files: [
      "src/styles.css — .ctx-mark ВСЕГДА #8a94a3; ctx-mark-c убран (Решение №7)",
      "src/content.js — идемпотентный scan (span.ctx-hl), selfChange, контейнеры, ≤3 меток, заливка fillOf",
      "src/core/normalize.js — + cleanUrl (только reply/thread/w)",
      "src/background.js — identity.url = cleanUrl(link); история {date, action, url, answer}",
      "dialog.html + src/dialog.js — «Точка встречи и история» (дата·кто·url·ответ) + селект ответа",
      "src/core/messaging.js — v07f4; manifest.json — 0.0.14",
      "src/dialog.js — единственный писатель: свежий loadDb в «Сохранить»/«Удалить»/селекте; селект «ожидание/да/нет» + маппинг старых",
      "src/background.js — подтверждён свежий loadDb в capture/NAME_HINT/MET_HINT; лог «db записана (total N)»",
      "src/content.js — только чтение (saveDb отсутствует)",
    ],
    test: [
      "ПКМ по дате → «автор сохранён/уже в базе + точка встречи», url без trackcode/recom",
      "▲ по одному на комментарий (серый), после обновления не прибавляется; ≤3 на карточку",
      "Заливка букв ника — цветом карточки; «грязь» → блеклость",
      "В диалоге — «Точка встречи и история» (последние 5); селект «ответ последнего контакта» пишет и сохраняет",
      "Карточка удаляется кнопкой; цвет/dirt живут без F5; одно окно; имя сохраняется",
      "После каждой записи — лог «db записана (total N)»; при удалении — «card cN удалена»; воскрешений/стираний нет",
      "Селект «ответ последнего контакта»: только ожидание/да/нет; старые значения маппятся",
    ],
    pass: "Пункты 1–7. R1/R2 в этот этап не входят (→ TASK-0012/v08r).",
    basis: "сводка проектировщика · фикс v07f4 (v07f3 = FAIL по D18) · Дополнение №3",
  },
];
