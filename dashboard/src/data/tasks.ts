...
export const tasks: TaskItem[] = [
  ...
  },
  {
    ...
    commit: "fix(ext): v07f5 — окна, надёжное удаление, логи, тип, живая перекраска [TASK-0011]",
    title: "v07f5: окна закрываются, удаление c25-подобных, «ясные» логи, тип персона/сообщество, живая перекраска",
    status: "queue",
    statusLabel: "NEW · исполнено · ждёт хэша",
    dep: "v07f4 (FAIL по пакету)",
    ...
    files: [
      "src/core/messaging.js — v07f5 + LOG; manifest.json — 0.0.15",
      "src/dialog.js — единственный писатель (loadDb→mutate→saveDb); тип персона/сообщество (R16); «ясные» логи LOG; окна закрываются",
      "src/background.js — card.type при изъятии (R16); обработчик LOG",
      "src/content.js — живая перекраска (D21): onChanged → buildIndex(newValue) → scan; wrap/fillOf из нового индекса",
      "dialog.html — селект «тип» (персона/сообщество) с явной разделительной линией",
    ],
    test: [
      ...
      "«Сохранить»/«Удалить» — окно закрывается, не висит",
      "Удаление c25-подобной (COMMENT с replyId) — удаляется всегда",
      "В SW-консоли — «card cN saved» / «card cN удалена» (ясные логи)",
      "Тип «персона/сообщество» (R16) в карточке сохраняется; при изъятии пишется верно",
      "Смена цвета/статуса → перекраска на странице без F5 (D21, fillOf из нового индекса)",
    ],
    pass: "Пункты 1–5. R1/R2 в этот этап не входят (→ TASK-0012/v08r).",
    basis: "сводка проектировщика · фикс v07f5 (v07f4 = FAIL по пакету) · Дополнение №3",
  },
];
