...
export const tasks: TaskItem[] = [
  ...
  },
  {
    ...
    commit: "feat(ext): v07g — стекло, скальпель, структура src/ui [TASK-0011]",
    title: "v07g: стекло, скальпель, структура src/ui — отрисовка вне DOM VK; вкл/выкл по значку",
    status: "queue",
    statusLabel: "NEW · исполнено · ждёт хэша",
    dep: "v07d (диагноз: React стирает узлы, marked 11 / wrappers 0)",
    goal: "React VK стирает внедрённые узлы. Решение: в DOM VK не добавляется НИЧЕГО — отрисовка на собственном стекле div#ctx-glass (fixed, поверх, pointer-events:none), маркеры ▲ — абсолютно позиционированы. Скальпель ВКЛ/ВЫКЛ — клик по значку (action.onClicked → CTX_TOGGLE); по умолчанию ВЫКЛ (расширение молчит); состояние в storage (переживает F5). Триггеры перерисовки (только при ВКЛ): SPA-навигация, observer (600 мс), resize, scroll (rAF), интервал 2 с, storage.onChanged. ПКМ-изъятие работает ВСЕГДА. Диалог, storage, единый писатель — не тронуты.",
    files: [
      "src/ui/layer.js — НОВЫЙ (СТЕКЛО): div#ctx-glass, draw() (ctx-g-hl ~25% / ctx-g-mark ▲ → OPEN_CARD), скальпель init/rescan/toggle, триггеры перерисовки",
      "src/content.js — тонкая точка входа: старт, лог db, CAPTURED, SAVE_AUTHOR, NAME_HINT, MET_HINT, CTX_TOGGLE → CTX_LAYER.init()/toggle(); изъятие работает всегда",
      ...
      "src/background.js — chrome.action.onClicked → CTX_TOGGLE; badge «on»/«» (setBadgeText)",
      "src/core/messaging.js — CTX_BUILD «v07g» + CTX_TOGGLE; manifest.json — 0.0.18, js: messaging, normalize, storage, ui/layer.js, content",
      "src/styles.css — #ctx-glass / .ctx-g-hl / .ctx-g-faded / .ctx-g-mark; wrap/unwrap/ctx-hl/ctx-mark/containerOf/perCard — в шредер",
      "docs/ARCHITECTURE_V07G_ADDENDUM.md — §H (оверлей-стекло) и §L (структура к факту)",
    ],
    test: [
      "vk.ru, по умолчанию: меток НЕТ, в консоли нет «observer: scan scheduled»",
      "Клик по значку → badge «on», «scalpel: ON», ▲ у сохранённых, подложка ~25%, «glass: N markers»",
      "Повторный клик → badge пустой, «scalpel: OFF», стекло очищено",
      "F5 со включённым скальпелем → метки вернулись (из storage)",
      "ПКМ-изъятие работает и при ВЫКЛ скальпеле",
      "Клик по ▲ → открывается карточка (OPEN_CARD)",
    ],
    pass: "Пункты 1–6; метки не стираются React на живых видах (стекло вне DOM VK — структурно).",
    basis: "сводка проектировщика · v07g (диагноз v07d) · Дополнение №3",
  },
];
