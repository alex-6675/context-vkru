...
export const tasks: TaskItem[] = [
  ...
  },
  {
    ...
    commit: "fix(ext): v07f3 — удаление, чистый url, прозрачность, точка встречи=коммент [TASK-0011]",
    title: "v07f3: удаление, чистый url, прозрачность ~25%, точка встречи = комментарий",
    status: "queue",
    statusLabel: "NEW · исполнено · ждёт хэша",
    dep: "v07f2 (FAIL по D13)",
    ...
    files: [
      ...
      "src/core/messaging.js — v07f3 + MET_HINT; manifest.json — 0.0.13",
      "src/content.js — заливка ~25% (hex+\"40\"); точка встречи (MET_HINT) из wall_comment_date",
      "src/dialog.js — удаление через filter + привязка; src/background.js — handleMetHint",
    ],
    test: [
    ...
    basis: "сводка проектировщика · фикс v07f3 (v07f2 = FAIL по D13) · Дополнение №3",
  },
];
