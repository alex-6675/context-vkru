...
===дописка от разраба ===
мой вопрос -- Что с версией Node ?

В показанном package.json нет 
===ответ разраба ===
Версия Node.js нигде не зафиксирована — ни в package.json, ни в .nvmrc. Однако анализ зависимостей показывает, что рабочий диапазон — Node 18, 20, 22+. Рекомендуется использовать Node 20 LTS или 22 LTS (актуальная LTS-ветка 2026 года). Для фиксации версии создан файл .nvmrc со значением 22, и сборка успешно прошла. Это требование касается только машины разработки, где происходит сборка React-дашборда, и не влияет на само расширение, которое работает в Edge без Node.
